...
export const _lastFiveEvents = Array.from({ length: 5 }, (_, index) => {
  const eventTypes = ['Conference', 'Meeting', 'Workshop', 'Presentation', 'Training'][index];
  return {
    id: `event-${index}`,
    type: eventTypes,
    status: ['en-cours', 'non-demarré', 'terminé', 'annulé', 'en-cours'][index],
    title: `Event ${index + 1}`,
    description: `Description for event ${index + 1}`,
    startDate: new Date(2024, 11, index + 1).toISOString(),
    organizer: `Organizer ${index + 1}`,
    location: ['Paris', 'London', 'Berlin', 'Madrid', 'Rome'][index],
