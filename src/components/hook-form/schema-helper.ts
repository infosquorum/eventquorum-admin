...
export const schemaHelper = {
  ...
  phoneNumber: (props?: { message?: MessageMapProps; isValid?: (text: string) => boolean }) =>
    zod
      .string({
        required_error: props?.message?.required ?? 'Numero de telephone est requis!',
        invalid_type_error: props?.message?.invalid_type ?? 'Numero de telephone invalide!',
      })
      .min(1, { message: props?.message?.required ?? 'Numero de telephone est requis!' })
      .refine((data) => props?.isValid?.(data), {
        message: props?.message?.invalid_type ?? 'Numero de telephone invalide!',
      }),
  /**
   * Date
      ...
      .transform((dateString, ctx) => {
        ...
        if (!dateString) {
          ctx.addIssue({
            ...
            message: props?.message?.required ?? 'La date est requise!',
          });
          return null;
        }
        ...
        if (!stringToDate.safeParse(date).success) {
          ctx.addIssue({
            ...
            message: props?.message?.invalid_type ?? 'Dtae invalide!!',
          });
        }
